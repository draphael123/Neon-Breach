...
export const brakeCorners=(()=>{const rate=(1.72-Math.min(level.topSpeed,80)*.009)*(1.06+(1-level.grip)*.25),turnR=level.topSpeed/rate,out=[];let inC=false,best=null;for(let i=0;i<=N;i++){const t=(i%N)/N;let d=yaw(t+.008)-yaw(t-.008);d=Math.atan2(Math.sin(d),Math.cos(d));const R=(.016*LENGTH)/Math.max(1e-4,Math.abs(d));if(R<turnR*1.05){if(!inC){inC=true;best={t,R,dir:Math.sign(d)}}else if(R<best.R)best={t,R,dir:Math.sign(d)}}else if(inC){inC=false;out.push(best)}}return out})();
