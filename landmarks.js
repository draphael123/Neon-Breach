...
export function buildLandmarks(W){
  ...
  brakeCorners.forEach((c,i)=>{
    ...
    else if(level.id==='blackout'){const t1=box(18,44,18,0,22,0,material(0x1a161c),g),t2=box(13,10,13,0,49,0,material(0x2a2530),g);const lamp=box(1.6,.8,1.6,0,54.3,0,glow(0xff2d2d,3),g);const steam=box(9,12,9,0,60,0,fog(0x8a8f9a,.14),g);update=now=>{lamp.visible=Math.sin(now*.004+i)>0;steam.scale.y=1+Math.sin(now*.002+i)*.3;steam.position.y=60+steam.scale.y*3}}
    else if(level.id==='solara'){box(10,34,10,-12,17,0,material(0x8d4c35),g);box(10,30,10,12,15,0,material(0xa95e3b),g);box(36,6,10,0,36,0,material(0x6f382e),g);const beacon=box(2,2,2,0,39.9,0,glow(0xffd166,3),g);update=now=>{beacon.material.emissiveIntensity=2+Math.sin(now*.005+i)*1.5}}
