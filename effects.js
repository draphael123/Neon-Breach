...
export function createAtmosphere(scene,theme={id:'midnight'}){
  ...
  function update(camera,now,visible,quality,speedRatio=0){rain.visible=visible;if(!visible)return;const rush=.65+Math.max(0,Math.min(1,speedRatio))*1.15,n=quality==='high'?count:160;material.opacity=(theme.id==='cryoline'?.22:.13)+rush*.07;geometry.setDrawRange(0,n*2);for(let i=0;i<n;i++){const s=seeds[i];let x=camera.position.x+(s[0]-.5)*70,y=camera.position.y+((s[1]*36-now*(theme.id==='cryoline'?.006:.018))%36+36)%36-10,z=camera.position.z+(s[2]-.5)*70,dx=-.2*rush,dy=-1.4*rush,dz=.12*rush;if(theme.id==='solara'){y=camera.position.y-2+s[1]*8;dx=2.4*rush;dy=.08;dz=.5*rush}else if(theme.id==='cryoline'){dx=.45*rush;dy=-.45*rush;dz=.2*rush}positions.set([x,y,z,x+dx,y+dy,z+dz],i*6)}geometry.attributes.position.needsUpdate=true}
  return {update};
}
